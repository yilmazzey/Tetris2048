...
class Game:
        ...
        while True:
            ...
            if stddraw.hasNextKeyTyped():
                ...
                #ADDING ROTATION KEYS
                    ...
                    current_tetromino.rotate(grid,current_tetromino, clockwise=False)
                elif key_typed == 'd':
                    current_tetromino.rotate(grid,current_tetromino, clockwise=True)   
            ...
            #tetrmonios keep moving down until the game is paused, over or won
            ...
            # Empty the list of tetrominos of if the game is restarted
            if self.restart:
                for a in range(0, 20):
                    for b in range(12):
                        grid.tile_matrix[a][b] = None
                #resetting the flags back to initial 
                self.restart = False 
    ...
    #-----------MERGE & CHAIN MERGE -----------------------      
    #perform merges and update the grid iteratively
    def scan_and_merge_grid(self,grid):
    ...
    #----------------ROW CLEANING-------------------------
    # If row is completely filled it returns true and deletes the rows
    ...
    def display_game_menu(self, grid_height, grid_width, grid):
        ...
        button_color = Color(28, 27, 25)
        text_color = Color(245, 243, 240)
        ...
        # If game is finished, allows user to restart game
            ...
            stddraw.setFontFamily('Helvetica')  # Set font family to Helvetica
            ...
            while True:
                ...
                if stddraw.mousePressed():
                    #getting the location of the most recent mouse click
                    mouse_x, mouse_y = stddraw.mouseX(), stddraw.mouseY()
                    if mouse_x >= button_blc_x and mouse_x <= button_blc_x + button_w:
                        if mouse_y >= button_blc_y and mouse_y <= button_blc_y + button_h:
                            text1_to_display = "Start Game"
    ...
    def speed_screen(self, grid, background_color, grid_width, grid_height, img_file, button_color):
        ...
        stddraw.setPenColor(Color(247, 240, 240))
