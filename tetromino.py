...
class Tetromino:
    ...
    def rotate(self, game_grid, current_tetromino, clockwise=True):
        ...
        if clockwise:
            ...
            for r in range(n):
                for c in range(n):
                    self.tile_matrix[n - 1 - c][r] = copy_matrix[r][c]

        # Adjust position if tiles are out of bounds
        for r in range(n):
            for c in range(n):
                if self.tile_matrix[r][c] is not None:
                    # Moves tiles to their new position
                    if clockwise:
                        self.tile_matrix[r][c].move(-r + c, -c + (n - 1 - r))
                    else:
                        self.tile_matrix[r][c].move(r - (n - 1 - c), c - r)

                    # Check if the tile is out of bounds and adjust
    ...
    def get_min_bounded_tile_matrix(self, return_position=False):
        ...
        # otherwise return the position of the bottom left cell in copy as well
             ...
             blc_position = cp.copy(self.bottom_left_corner)
